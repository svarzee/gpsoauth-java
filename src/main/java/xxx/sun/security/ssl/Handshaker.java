...
import xxx.sun.security.ssl.CipherSuite.MacAlg;
import xxx.sun.security.ssl.CipherSuite.PRF;
import xxx.sun.security.ssl.HandshakeMessage.Finished;
...
import static xxx.sun.security.ssl.CipherSuite.CipherType.AEAD_CIPHER;
import static xxx.sun.security.ssl.CipherSuite.CipherType.BLOCK_CIPHER;
...
abstract class Handshaker {
     ...
     */
    private xxx.sun.security.ssl.CipherSuiteList activeCipherSuites;
    ...
    static final boolean allowUnsafeRenegotiation = Debug.getBooleanProperty(
                    "xxx.sun.security.ssl.allowUnsafeRenegotiation", false);
    ...
    static final boolean allowLegacyHelloMessages = Debug.getBooleanProperty(
                    "xxx.sun.security.ssl.allowLegacyHelloMessages", true);
    ...
    private void init(SSLContextImpl context, ProtocolList enabledProtocols,
        ...
        if (conn != null) {
            algorithmConstraints = new xxx.sun.security.ssl.SSLAlgorithmConstraints(conn, true);
        } else {        // engine != null
            algorithmConstraints = new xxx.sun.security.ssl.SSLAlgorithmConstraints(engine, true);
        }
    }
     ...
     * handshake is not yet in progress).
     */
    void setEnabledCipherSuites(xxx.sun.security.ssl.CipherSuiteList enabledCipherSuites) {
        activeCipherSuites = null;
        activeProtocols = null;
    ...
    void setAlgorithmConstraints(AlgorithmConstraints algorithmConstraints) {
        ...
        this.algorithmConstraints =
            new xxx.sun.security.ssl.SSLAlgorithmConstraints(algorithmConstraints);
     ...
     * Does not check if the required server certificates are available.
     */
    final static boolean isNegotiable(xxx.sun.security.ssl.CipherSuiteList proposed, CipherSuite s) {
     ...
     */
    xxx.sun.security.ssl.CipherSuiteList getActiveCipherSuites() {
        if (activeCipherSuites == null) {
            if (activeProtocols == null) {
                activeProtocols = getActiveProtocols();
            }
            ...
            activeCipherSuites = new xxx.sun.security.ssl.CipherSuiteList(suites);
        }
     ...
     */
    void process_record(xxx.sun.security.ssl.InputRecord r, boolean expectingFinished)
            throws IOException {
