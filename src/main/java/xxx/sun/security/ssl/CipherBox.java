...
import xxx.sun.security.ssl.CipherSuite.CipherType;
...
import static xxx.sun.security.ssl.CipherSuite.B_NULL;
import static xxx.sun.security.ssl.CipherSuite.CipherType.*;
...
final class CipherBox {
    ...
    final static xxx.sun.security.ssl.CipherBox NULL = new xxx.sun.security.ssl.CipherBox();
    ...
    private CipherBox(ProtocolVersion protocolVersion, BulkCipher bulkCipher,
        ...
        try {
            ...
            this.cipher = xxx.sun.security.ssl.JsseJce.getCipher(bulkCipher.transformation);
            this.mode = encrypt ? Cipher.ENCRYPT_MODE : Cipher.DECRYPT_MODE;

            if (random == null) {
                random = xxx.sun.security.ssl.JsseJce.getSecureRandom();
            }
            this.random = random;
     ...
     */
    static xxx.sun.security.ssl.CipherBox newCipherBox(ProtocolVersion version, BulkCipher cipher,
                                                   SecretKey key, IvParameterSpec iv, SecureRandom random,
                                                   boolean encrypt) throws NoSuchAlgorithmException {
        ...
        if (cipher == B_NULL) {
            ...
            return new xxx.sun.security.ssl.CipherBox(version, cipher, key, iv, random, encrypt);
        }
    }
     ...
     * @return the explicit nonce size of the cipher.
     */
    int applyExplicitNonce(xxx.sun.security.ssl.Authenticator authenticator, byte contentType,
                           ByteBuffer bb) throws BadPaddingException {
        switch (cipherType) {
            case BLOCK_CIPHER:
                // sanity check length of the ciphertext
                int tagLen = (authenticator instanceof xxx.sun.security.ssl.MAC) ?
                                    ((xxx.sun.security.ssl.MAC)authenticator).MAClen() : 0;
                if (tagLen != 0) {
     ...
     * @return the explicit nonce size of the cipher.
     */
    int applyExplicitNonce(xxx.sun.security.ssl.Authenticator authenticator,
     ...
     */
    byte[] createExplicitNonce(xxx.sun.security.ssl.Authenticator authenticator,
