...
import xxx.sun.security.ssl.CipherSuite.PRF;
...
import static xxx.sun.security.ssl.CipherSuite.PRF.P_NONE;
...
static final class ClientHello extends HandshakeMessage {
    ...
    xxx.sun.security.ssl.SessionId sessionId;
    private xxx.sun.security.ssl.CipherSuiteList cipherSuites;
    byte[]              compression_methods;

    xxx.sun.security.ssl.HelloExtensions extensions = new xxx.sun.security.ssl.HelloExtensions();

    private final static byte[]  NULL_COMPRESSION = new byte[] {0};

    ClientHello(SecureRandom generator, ProtocolVersion protocolVersion,
                xxx.sun.security.ssl.SessionId sessionId, xxx.sun.security.ssl.CipherSuiteList cipherSuites) {
    ...
    ClientHello(HandshakeInStream s, int messageLength) throws IOException {
        ...
        sessionId = new xxx.sun.security.ssl.SessionId(s.getBytes8());
        sessionId.checkLength(protocolVersion);
        cipherSuites = new xxx.sun.security.ssl.CipherSuiteList(s);
        compression_methods = s.getBytes8();
        if (messageLength() != messageLength) {
            extensions = new xxx.sun.security.ssl.HelloExtensions(s);
        }
    }

    xxx.sun.security.ssl.CipherSuiteList getCipherSuites() {
        return cipherSuites;
    }

    // add renegotiation_info extension
    void addRenegotiationInfoExtension(byte[] clientVerifyData) {
        xxx.sun.security.ssl.HelloExtension renegotiationInfo = new xxx.sun.security.ssl.RenegotiationInfoExtension(
    ...
    void addSignatureAlgorithmsExtension(
        ...
        xxx.sun.security.ssl.HelloExtension signatureAlgorithm =
                new xxx.sun.security.ssl.SignatureAlgorithmsExtension(algorithms);
        extensions.add(signatureAlgorithm);
    }

    void addExtendedMasterSecretExtension() {
        extensions.add(new xxx.sun.security.ssl.ExtendedMasterSecretExtension());
    }
...
class ServerHello extends HandshakeMessage
{
    ...
    ServerHello(HandshakeInStream input, int messageLength)
        ...
        sessionId = new xxx.sun.security.ssl.SessionId(input.getBytes8());
        sessionId.checkLength(protocolVersion);
        cipherSuite = CipherSuite.valueOf(input.getInt8(), input.getInt8());
        compression_method = (byte)input.getInt8();
        if (messageLength() != messageLength) {
            extensions = new xxx.sun.security.ssl.HelloExtensions(input);
        }
    }
...
class DH_ServerKeyExchange extends ServerKeyExchange
{
    ...
    private xxx.sun.security.ssl.SignatureAndHashAlgorithm preferableSignatureAlgorithm;

    /*
     * Construct from initialized DH key object, for DH_anon
     * key exchange.
     */
    DH_ServerKeyExchange(xxx.sun.security.ssl.DHCrypt obj, ProtocolVersion protocolVersion) {
        this.protocolVersion = protocolVersion;
     ...
     */
    DH_ServerKeyExchange(xxx.sun.security.ssl.DHCrypt obj, PrivateKey key, byte clntNonce[],
                         byte svrNonce[], SecureRandom sr,
                         xxx.sun.security.ssl.SignatureAndHashAlgorithm signAlgorithm,
    ...
    DH_ServerKeyExchange(HandshakeInStream input, PublicKey publicKey,
                         ...
                         Collection<xxx.sun.security.ssl.SignatureAndHashAlgorithm> localSupportedSignAlgs,
                         ProtocolVersion protocolVersion)
            throws IOException, GeneralSecurityException {
        ...
        if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
            ...
            preferableSignatureAlgorithm =
                xxx.sun.security.ssl.SignatureAndHashAlgorithm.valueOf(hash, signature, 0);
    ...
    private void setValues(xxx.sun.security.ssl.DHCrypt obj) {
    ...
    int messageLength() {
        ...
        if (signature != null) {
            if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
                temp += xxx.sun.security.ssl.SignatureAndHashAlgorithm.sizeInRecord();
            }
...
class ECDH_ServerKeyExchange extends ServerKeyExchange {
    ...
    int messageLength() {
        ...
        if (signatureBytes != null) {
            ...
            if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
                sigLen += xxx.sun.security.ssl.SignatureAndHashAlgorithm.sizeInRecord();
            }
        }
...
class CertificateRequest extends HandshakeMessage
{
    ...
    HandshakeMessage.DistinguishedName authorities [];         // 3 to 2^16 - 1
    ...
    private Collection<xxx.sun.security.ssl.SignatureAndHashAlgorithm> algorithms;

    // length of supported_signature_algorithms
    private int algorithmsLen;

    CertificateRequest(X509Certificate ca[], KeyExchange keyExchange,
            Collection<xxx.sun.security.ssl.SignatureAndHashAlgorithm> signAlgs,
            ProtocolVersion protocolVersion) throws IOException {

        this.protocolVersion = protocolVersion;

        // always use X500Principal
        authorities = new HandshakeMessage.DistinguishedName[ca.length];
        for (int i = 0; i < ca.length; i++) {
            X500Principal x500Principal = ca[i].getSubjectX500Principal();
            authorities[i] = new HandshakeMessage.DistinguishedName(x500Principal);
        }
        ...
        if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
            ...
            algorithms = new ArrayList<xxx.sun.security.ssl.SignatureAndHashAlgorithm>(signAlgs);
            algorithmsLen =
                xxx.sun.security.ssl.SignatureAndHashAlgorithm.sizeInRecord() * algorithms.size();
        } else {
            algorithms = new ArrayList<xxx.sun.security.ssl.SignatureAndHashAlgorithm>();
            algorithmsLen = 0;
        }
    }
    ...
    CertificateRequest(HandshakeInStream input,
        ...
        if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
            ...
            while (remains > 1) {    // needs at least two bytes
                ...
                xxx.sun.security.ssl.SignatureAndHashAlgorithm algorithm =
                    xxx.sun.security.ssl.SignatureAndHashAlgorithm.valueOf(hash, signature,
                                                                ++sequence);
        ...
        ArrayList<HandshakeMessage.DistinguishedName> v = new ArrayList<>();
        while (len >= 3) {
            HandshakeMessage.DistinguishedName dn = new HandshakeMessage.DistinguishedName(input);
            v.add(dn);
            len -= dn.length();
        }
        ...
        authorities = v.toArray(new HandshakeMessage.DistinguishedName[v.size()]);
    }
    ...
    Collection<xxx.sun.security.ssl.SignatureAndHashAlgorithm> getSignAlgorithms() {
        return algorithms;
    }
    ...
    void print(PrintStream s) throws IOException {
        ...
        if (debug != null && Debug.isOn("verbose")) {
            ...
            if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
                ...
                for (xxx.sun.security.ssl.SignatureAndHashAlgorithm signAlg : algorithms) {
                    if (opened) {
...
static final class CertificateVerify extends HandshakeMessage {
    ...
    private xxx.sun.security.ssl.SignatureAndHashAlgorithm preferableSignatureAlgorithm = null;
    ...
    CertificateVerify(HandshakeInStream input,
        ...
        if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
            ...
            preferableSignatureAlgorithm =
                xxx.sun.security.ssl.SignatureAndHashAlgorithm.valueOf(hashAlg, signAlg, 0);
     ...
     */
    xxx.sun.security.ssl.SignatureAndHashAlgorithm getPreferableSignatureAlgorithm() {
...
static final class Finished extends HandshakeMessage {
    ...
    private static void updateDigest(MessageDigest md, byte[] sender,
        ...
        md.update(sender);
        HandshakeMessage.CertificateVerify.updateDigest(md, pad1, pad2, masterSecret);
    }
