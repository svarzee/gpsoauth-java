...
import xxx.sun.security.ssl.SignatureAndHashAlgorithm.HashAlgorithm;
import xxx.sun.security.ssl.SignatureAndHashAlgorithm.SignatureAlgorithm;
...
import static xxx.sun.security.ssl.CipherSuite.KeyExchange.*;
 ...
 */
final class ServerHandshaker extends xxx.sun.security.ssl.Handshaker {
     ...
     */
    private xxx.sun.security.ssl.DHCrypt dh;
    ...
    private xxx.sun.security.ssl.EllipticCurvesExtension requestedCurves;

    // the preferable signature algorithm used by ServerKeyExchange message
    xxx.sun.security.ssl.SignatureAndHashAlgorithm preferableSignatureAlgorithm;
    ...
    private static final AlgorithmConstraints legacyAlgorithmConstraints =
            new LegacyAlgorithmConstraints(
                    ...
                    new xxx.sun.security.ssl.SSLAlgorithmDecomposer());
    ...
    ServerHandshaker(SSLSocketImpl socket, SSLContextImpl context,
                     xxx.sun.security.ssl.ProtocolList enabledProtocols, byte clientAuth,
        ...
        switch (type) {
            ...
            case HandshakeMessage.ht_certificate:
                if (doClientAuth == SSLEngineImpl.clauth_none) {
                    fatalSE(xxx.sun.security.ssl.Alerts.alert_unexpected_message,
            ...
            case HandshakeMessage.ht_client_key_exchange:
                ...
                case K_RSA_EXPORT:
                     ...
                     * certificates/keys.
                     */
                    xxx.sun.security.ssl.RSAClientKeyExchange pms = new xxx.sun.security.ssl.RSAClientKeyExchange(
                ...
                case K_DH_ANON:
                     ...
                     */
                    xxx.sun.security.ssl.DHClientKeyExchange dhcke = new xxx.sun.security.ssl.DHClientKeyExchange(input);
                ...
                case K_ECDHE_ECDSA:
                case K_ECDH_ANON:
                    xxx.sun.security.ssl.ECDHClientKeyExchange ecdhcke =
                        new xxx.sun.security.ssl.ECDHClientKeyExchange(input);
    ...
    private void clientHello(ClientHello mesg) throws IOException {
        ...
        if (rejectClientInitiatedRenego && !isInitialHandshake &&
            ...
            fatalSE(xxx.sun.security.ssl.Alerts.alert_handshake_failure,
                "Client initiated renegotiation is not allowed");
        }

        // check the server name indication if required
        xxx.sun.security.ssl.ServerNameExtension clientHelloSNIExt = (xxx.sun.security.ssl.ServerNameExtension)
                    mesg.extensions.get(xxx.sun.security.ssl.ExtensionType.EXT_SERVER_NAME);
        if (!sniMatchers.isEmpty()) {
            // we do not reject client without SNI extension
            if (clientHelloSNIExt != null &&
                        !clientHelloSNIExt.isMatched(sniMatchers)) {
                fatalSE(xxx.sun.security.ssl.Alerts.alert_unrecognized_name,
        ...
        xxx.sun.security.ssl.CipherSuiteList cipherSuites = mesg.getCipherSuites();
        if (cipherSuites.contains(xxx.sun.security.ssl.CipherSuite.C_SCSV)) {
        ...
        xxx.sun.security.ssl.RenegotiationInfoExtension clientHelloRI = (xxx.sun.security.ssl.RenegotiationInfoExtension)
                    mesg.extensions.get(xxx.sun.security.ssl.ExtensionType.EXT_RENEGOTIATION_INFO);
        ...
        if (!renegotiationIndicated || !secureRenegotiation) {
            if (isInitialHandshake) {
                ...
                if (activeProtocolVersion.v >= ProtocolVersion.TLS10.v) {
                    ...
                    warningSE(xxx.sun.security.ssl.Alerts.alert_no_renegotiation);
        ...
        if (useExtendedMasterSecret) {
            xxx.sun.security.ssl.ExtendedMasterSecretExtension extendedMasterSecretExtension =
                    (xxx.sun.security.ssl.ExtendedMasterSecretExtension)mesg.extensions.get(
                            xxx.sun.security.ssl.ExtensionType.EXT_EXTENDED_MASTER_SECRET);
        ...
        svr_random = new xxx.sun.security.ssl.RandomCookie(sslContext.getSecureRandom());
        ...
        if (mesg.sessionId.length() != 0) {
            ...
            xxx.sun.security.ssl.SSLSessionImpl previous = ((xxx.sun.security.ssl.SSLSessionContextImpl)sslContext
            ...
            if (previous != null) {
                ...
                if (resumingSession) {
                    xxx.sun.security.ssl.CipherSuite suite = previous.getSuite();
        ...
        if (session == null) {
            ...
            requestedCurves = (xxx.sun.security.ssl.EllipticCurvesExtension)
                        mesg.extensions.get(xxx.sun.security.ssl.ExtensionType.EXT_ELLIPTIC_CURVES);

            // We only need to handle the "signature_algorithm" extension
            // for full handshakes and TLS 1.2 or later.
            if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
                xxx.sun.security.ssl.SignatureAlgorithmsExtension signAlgs =
                    (xxx.sun.security.ssl.SignatureAlgorithmsExtension)mesg.extensions.get(
                                    xxx.sun.security.ssl.ExtensionType.EXT_SIGNATURE_ALGORITHMS);
                if (signAlgs != null) {
                    Collection<xxx.sun.security.ssl.SignatureAndHashAlgorithm> peerSignAlgs =
                    ...
                    Collection<xxx.sun.security.ssl.SignatureAndHashAlgorithm>
                        supportedPeerSignAlgs =
                            xxx.sun.security.ssl.SignatureAndHashAlgorithm.getSupportedAlgorithms(
            ...
            session = new xxx.sun.security.ssl.SSLSessionImpl(protocolVersion, xxx.sun.security.ssl.CipherSuite.C_NULL,
        ...
        if (secureRenegotiation) {
            ...
            xxx.sun.security.ssl.HelloExtension serverHelloRI = new xxx.sun.security.ssl.RenegotiationInfoExtension(
                                        clientVerifyData, serverVerifyData);
        ...
        if (!sniMatchers.isEmpty() && clientHelloSNIExt != null) {
            ...
            if (!resumingSession) {
                xxx.sun.security.ssl.ServerNameExtension serverHelloSNI = new xxx.sun.security.ssl.ServerNameExtension();
                m1.extensions.add(serverHelloSNI);
            }
        }

        if (session.getUseExtendedMasterSecret()) {
            m1.extensions.add(new xxx.sun.security.ssl.ExtendedMasterSecretExtension());
        }
        ...
        case K_KRB5_EXPORT:
            ...
            break;
        case K_RSA_EXPORT:
            if (xxx.sun.security.ssl.JsseJce.getRSAKeyLength(certs[0].getPublicKey()) > 512) {
                try {
        ...
        if (doClientAuth != SSLEngineImpl.clauth_none &&
            ...
            Collection<xxx.sun.security.ssl.SignatureAndHashAlgorithm> localSignAlgs = null;
            if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
                ...
                Set<String> localHashAlgs =
                    xxx.sun.security.ssl.SignatureAndHashAlgorithm.getHashAlgorithmNames(
                        localSignAlgs);
    ...
    private void chooseCipherSuite(ClientHello mesg) throws IOException {
        xxx.sun.security.ssl.CipherSuiteList prefered;
        xxx.sun.security.ssl.CipherSuiteList proposed;
        ...
        List<xxx.sun.security.ssl.CipherSuite> legacySuites = new ArrayList<>();
        for (xxx.sun.security.ssl.CipherSuite suite : prefered.collection()) {
        ...
        for (xxx.sun.security.ssl.CipherSuite suite : legacySuites) {
        ...
        fatalSE(xxx.sun.security.ssl.Alerts.alert_handshake_failure, "no cipher suites in common");
    }
     ...
     */
    boolean trySetCipherSuite(xxx.sun.security.ssl.CipherSuite suite) {
        /*
        ...
        Collection<xxx.sun.security.ssl.SignatureAndHashAlgorithm> supportedSignAlgs = null;
        if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
            if (peerSupportedSignAlgs != null) {
                supportedSignAlgs = peerSupportedSignAlgs;
            } else {
                xxx.sun.security.ssl.SignatureAndHashAlgorithm algorithm = null;
                ...
                switch (keyExchange) {
                    ...
                    case K_ECDH_RSA:
                    case K_ECDHE_RSA:
                        algorithm = xxx.sun.security.ssl.SignatureAndHashAlgorithm.valueOf(
                ...
                if (algorithm == null) {
                    supportedSignAlgs =
                        Collections.<xxx.sun.security.ssl.SignatureAndHashAlgorithm>emptySet();
                } else {
                    supportedSignAlgs =
                        new ArrayList<xxx.sun.security.ssl.SignatureAndHashAlgorithm>(1);
        ...
        case K_DHE_RSA:
            ...
            if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
                preferableSignatureAlgorithm =
                    xxx.sun.security.ssl.SignatureAndHashAlgorithm.getPreferableAlgorithm(
    ...
    private void setupEphemeralDHKeys(boolean export, Key key) {
        ...
        dh = new xxx.sun.security.ssl.DHCrypt(keySize, sslContext.getSecureRandom());
    }
    ...
    private boolean setupEphemeralECDHKeys() {
        int index = (requestedCurves != null) ?
                ...
                xxx.sun.security.ssl.EllipticCurvesExtension.getActiveCurves(algorithmConstraints);
        if (index < 0) {
    ...
    private boolean setupPrivateKeyAndChain(String algorithm) {
        ...
        if (keyAlgorithm.equals("EC")) {
            ...
            int id = xxx.sun.security.ssl.EllipticCurvesExtension.getCurveIndex(params);
            if ((id <= 0) || !xxx.sun.security.ssl.EllipticCurvesExtension.isSupported(id) ||
     ...
     * pre-master secret.
     */
    private SecretKey clientKeyExchange(xxx.sun.security.ssl.DHClientKeyExchange mesg)
            throws IOException {
    ...
    private SecretKey clientKeyExchange(xxx.sun.security.ssl.ECDHClientKeyExchange mesg)
            throws IOException {
    ...
    private void clientCertificateVerify(CertificateVerify mesg)
        ...
        if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
            xxx.sun.security.ssl.SignatureAndHashAlgorithm signAlg =
            ...
            String hashAlg =
                xxx.sun.security.ssl.SignatureAndHashAlgorithm.getHashAlgorithmName(signAlg);
        ...
        try {
            ...
            if (valid == false) {
                fatalSE(xxx.sun.security.ssl.Alerts.alert_bad_certificate,
    ...
    private void clientFinished(Finished mesg) throws IOException {
        ...
        if (!resumingSession && session.isRejoinable()) {
            ((xxx.sun.security.ssl.SSLSessionContextImpl)sslContext.engineGetServerSessionContext())
                .put(session);
            if (debug != null && Debug.isOn("session")) {
                System.out.println(
    ...
    void handshakeAlert(byte description) throws SSLProtocolException {
        ...
        String message = xxx.sun.security.ssl.Alerts.alertDescription(description);
         ...
         */
        if ((description == xxx.sun.security.ssl.Alerts.alert_no_certificate) &&
     ...
     */
    private SecretKey clientKeyExchange(xxx.sun.security.ssl.RSAClientKeyExchange mesg)
            throws IOException {
    ...
    private void clientCertificate(CertificateMsg mesg) throws IOException {
        ...
        try {
            ...
            fatalSE(xxx.sun.security.ssl.Alerts.alert_certificate_unknown, e);
        }
