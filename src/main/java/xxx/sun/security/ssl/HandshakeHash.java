...
final class HandshakeHash {
    ...
    void protocolDetermined(ProtocolVersion pv) {
        ...
        switch (version) {
            case 1:
                ...
                try {
                    md5 = xxx.sun.security.ssl.CloneableDigest.getDigest("MD5", clonesNeeded);
                    sha = xxx.sun.security.ssl.CloneableDigest.getDigest("SHA", clonesNeeded);
    ...
    void setFinishedAlg(String s) {
        ...
        try {
            ...
            finMD = xxx.sun.security.ssl.CloneableDigest.getDigest(normalizeAlgName(s), 4);
        } catch (NoSuchAlgorithmException e) {
            throw new Error(e);
        }
...
final class CloneableDigest extends MessageDigest implements Cloneable {
    ...
    private CloneableDigest(MessageDigest digest, int n, String algorithm)
        ...
        digests[0] = digest;
        for (int i = 1; i < n; i++) {
            digests[i] = xxx.sun.security.ssl.JsseJce.getMessageDigest(algorithm);
        }
    }
    ...
    static MessageDigest getDigest(String algorithm, int n)
            throws NoSuchAlgorithmException {
        MessageDigest digest = xxx.sun.security.ssl.JsseJce.getMessageDigest(algorithm);
        try {
            digest.clone();
            // already cloneable, use it
            return digest;
        } catch (CloneNotSupportedException e) {
            return new xxx.sun.security.ssl.CloneableDigest(digest, n, algorithm);
        }
    }
