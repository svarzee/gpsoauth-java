...
final public class SSLEngineImpl extends SSLEngine {
    ...
    private xxx.sun.security.ssl.SSLSessionImpl sess;
    private volatile xxx.sun.security.ssl.SSLSessionImpl handshakeSession;
    ...
    xxx.sun.security.ssl.EngineInputRecord inputRecord;
    xxx.sun.security.ssl.EngineOutputRecord outputRecord;
    ...
    private void init(SSLContextImpl ctx) {
        ...
        sslContext = ctx;
        sess = xxx.sun.security.ssl.SSLSessionImpl.nullSession;
        ...
        readAuthenticator = xxx.sun.security.ssl.MAC.NULL;
        writeCipher = CipherBox.NULL;
        writeAuthenticator = xxx.sun.security.ssl.MAC.NULL;
         ...
         */
        outputRecord =
            new xxx.sun.security.ssl.EngineOutputRecord(xxx.sun.security.ssl.Record.ct_application_data, this);
        inputRecord = new xxx.sun.security.ssl.EngineInputRecord(this);
    ...
    private SSLEngineResult readNetRecord(EngineArgs ea) throws IOException {
        ...
        if (packetLen > sess.getPacketBufferSize()) {
            if (packetLen > xxx.sun.security.ssl.Record.maxLargeRecordSize) {
                throw new SSLProtocolException(
                    "Input SSL/TLS record too big: max = " +
                    xxx.sun.security.ssl.Record.maxLargeRecordSize +
                    " len = " + packetLen);
            } else {
         ...
         */
        if ((packetLen - xxx.sun.security.ssl.Record.headerSize) > ea.getAppRemaining()) {
        ...
        if (getConnectionState() != cs_ERROR) {
            ...
            try {
                ...
                byte alertType = (inputRecord.contentType() ==
                    xxx.sun.security.ssl.Record.ct_handshake) ?
    ...
    public SSLEngineResult wrap(ByteBuffer [] appData,
         ...
         */
        if (netData.remaining() < xxx.sun.security.ssl.EngineOutputRecord.maxRecordSize) {
            return new SSLEngineResult(
     ...
     */
    private HandshakeStatus writeRecord(xxx.sun.security.ssl.EngineOutputRecord eor,
     ...
     */
    void writeRecord(xxx.sun.security.ssl.EngineOutputRecord eor) throws IOException {
        // eventually compress as well.
    ...
    private boolean checkSequenceNumber(Authenticator authenticator, byte type)
         ...
         */
        if (connectionState >= cs_ERROR || authenticator == xxx.sun.security.ssl.MAC.NULL) {
            return false;
        }
    ...
    synchronized void setHandshakeSession(xxx.sun.security.ssl.SSLSessionImpl session) {
    ...
    private void sendAlert(byte level, byte description) {
        ...
        xxx.sun.security.ssl.EngineOutputRecord r = new xxx.sun.security.ssl.EngineOutputRecord(xxx.sun.security.ssl.Record.ct_alert, this);
